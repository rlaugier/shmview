...
class MyWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        spacers_pos = np.load("/dev/shm/spacers.npy")
        self.spacers = []
        for apos in spacers_pos:
            aline = pg.InfiniteLine(angle=90,movable=False)
            aline.setPos(apos)
            self.spacers.append(aline)
        for aspacer in self.spacers:
            self.gView_shm.addItem(aspacer)
